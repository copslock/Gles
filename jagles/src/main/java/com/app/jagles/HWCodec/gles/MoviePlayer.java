...
public class MoviePlayer {
    ...
    public interface TimeStamp {
        void TimeStamp(long presentationTimeUsec);
    }
    ...
    public MoviePlayer(File sourceFile, Surface outputSurface, FrameCallback frameCallback)
        ...
        try {
            ...
            mSampeletime = format.getLong(android.media.MediaFormat.KEY_DURATION);
            if (VERBOSE) {
    ...
    public void Pause(){
        ...
        mIsContinueRequested = false;
    }

    public void Continue(){
        if(mIsPauseRequested)
            mIsContinueRequested = true;
    }
    ...
    public void play() throws IOException {
        ...
        try {
            ...
            audio_decoder = MediaCodec.createDecoderByType(mime);
            audio_decoder.configure(format,null,null,0);
            audio_decoder.start();

            doExtract(extractor, trackIndex, decoder,audiotrackindex,audio_decoder, mFrameCallback);
        } finally {
     ...
     */
    private static int selectTrack(MediaExtractor extractor,boolean isaudio) {
    ...
    private void doExtract(MediaExtractor extractor, int trackIndex, MediaCodec decoder,
                           int audiotrackIndex,MediaCodec audio_decoder,FrameCallback frameCallback) {
        ...
        ByteBuffer[] audio_decoderInputBuffers = audio_decoder.getInputBuffers();
        int inputChunk = 0;
        ...
        while (!outputDone) {
            ...
            if(mIsPauseRequested&&!mIsContinueRequested)
            {
                try
                {
                    Thread.sleep(20,0);
            ...
            if(mIsPauseRequested&&mIsContinueRequested)
            {
                ...
                mIsContinueRequested = false;
                frameCallback.ResetPosition();
            }
            // Feed more data to the decoder.
            if (!inputDone) {
                int inputBufIndex = decoder.dequeueInputBuffer(TIMEOUT_USEC);
                int audio_inputBufIndex = audio_decoder.dequeueInputBuffer(TIMEOUT_USEC);
                if (inputBufIndex >= 0 || audio_inputBufIndex>=0) {
            ...
            if (!outputDone) {
                ...
                if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    // the buffers.
                    if(doRender && mTimeStamp!=null)
                    {
                        synchronized (this) {
                            mTimeStamp.TimeStamp(mBufferInfo.presentationTimeUs);
                        }
                    }

                    if (doRender && frameCallback != null) {
                        frameCallback.preRender(mBufferInfo.presentationTimeUs);
                        //Log.d("honglee","presentationTimeUs:"+mBufferInfo.presentationTimeUs);
                    }
                    ...
                    if(doRender && mSeekTime>0)
                    {
                        synchronized (this)
                        {
                            extractor.seekTo(mSeekTime, MediaExtractor.SEEK_TO_CLOSEST_SYNC);
                            mSeekTime = 0;
                            decoder.flush();
                            frameCallback.ResetPosition();
                        }
    ...
    public static class PlayTask implements Runnable {
        ...
        private static class LocalHandler extends Handler {
            ...
            public LocalHandler(Looper loop) {
                super(loop);
            }
        }
    }
