...
void eTB_GLSL__print_uniforms(GLuint program)
{
	...
	for (i = 0; i < uniform_count; i++) {
		...
		for (j = 0; j < sizeof(type_set) / sizeof(struct glsl_type_set); j++) {
			...
			if (size > 1)
				LOGV("Uniform %d (loc=%d):\t%20s %-20s <Size: %d>\n",
					i, location, type_name, name, size);
			else
				LOGV("Uniform %d (loc=%d):\t%20s %-20s\n",
...
void ksPerspective(KSMatrix4 *result, float fovy, float aspect, float nearZ, float farZ) {
    ...
    //printf("ksPerspective.............................\n");
    //return;
    frustumH = tanf(fovy / 360.0f * M_PI) * nearZ;
    frustumW = frustumH * aspect;

    ksFrustum(result, -frustumW, frustumW, -frustumH, frustumH, nearZ, farZ);
    //ksFrustum(result, -aspect, aspect, -1, 1, 1, 7);
//    KSVec3 eye,target,up;
