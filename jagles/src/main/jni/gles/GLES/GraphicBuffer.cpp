...
bool setFuncPtr (Func*& funcPtr, const DynamicLibrary& lib, const string& symname)
{
	bool isSupport = lib.isSupport;
	if (isSupport) {
...
GraphicBuffer::GraphicBuffer(uint32_t width, uint32_t height, PixelFormat format, uint32_t usage):
    library("libui.so")
{
    bool isSupport = setFuncPtr(functions.constructor, library, "_ZN7android13GraphicBufferC1Ejjij");
	if (!isSupport) {
		return;
	}
...
bool GraphicBuffer::IsSupportDirectTexture()
{
	return library.isSupport;
}